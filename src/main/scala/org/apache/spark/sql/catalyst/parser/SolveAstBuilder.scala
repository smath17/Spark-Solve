 ...
 * TableIdentifier.
 */
class SolveAstBuilder extends SqlBaseBaseVisitor[AnyRef] with SQLConfHelper with Logging {
   ...
   * ******************************************************************************************** */
        ...
        // override def visitSolveQuerySpecification( ctx: SolveQuerySpecificationContext): LogicalPlan = withOrigin(ctx) {
                // withSolveQuerySpecification()
        // }

        private def withSolveQuerySpecification(
                                               ctx: ParserRuleContext,
                                               solveClause: SolveClauseContext,
                                               objectiveClause: ObjectiveClauseContext,
                                               subjectToClause: SubjecttoClauseContext,
                                               relation: LogicalPlan
                                               ): LogicalPlan = withOrigin(ctx) {
                val expressions = visitNamedExpressionSeq(solveClause.namedExpressionSeq)
                val withObjective = visitSelectFromHelper(objectiveClause.selectFromHelper())
                val objDirection = objectiveClause.direction
                ...
                def createProject() = if (namedExpressions.nonEmpty) {
                        Project(namedExpressions, withObjective)
                } else {
                        withObjective
                }

                val withProject = createProject()

        }

        override def visitSelectFromHelper(ctx: SelectFromHelperContext): LogicalPlan = withOrigin(ctx) {
                visitNamedExpressionSeq(ctx.selectClause().namedExpressionSeq())
                visitFromClause(ctx.fromClause())
        }
        ...
        private def withHints(
                ...
                ctx.hintStatements.asScala.reverse.foreach { stmt =>
                        plan = UnresolvedHint(stmt.hintName.getText,
                                stmt.parameters.asScala.map(expression), plan)
                }
                plan
        }
                                ...
                                Option(ctx.quantifier).map(_.getType) match {
                                        case Some(SqlBaseParser.ANY) | Some(SqlBaseParser.SOME) =>
                                                ...
                                                if (expressions.forall(_.foldable) && expressions.forall(_.dataType == StringType)) {
                                                        ...
                                                        ctx.expression.asScala.map(expression)
                                                          .map(p => invertIfNotDefined(new Like(e, p))).reduceLeft(Or)
                                                }
                                        case Some(SqlBaseParser.ALL) =>
                                                ...
                                                if (expressions.forall(_.foldable) && expressions.forall(_.dataType == StringType)) {
                                                        ...
                                                        ctx.expression.asScala.map(expression)
                                                          .map(p => invertIfNotDefined(new Like(e, p))).reduceLeft(And)
                                                }
                                        case _ =>
   ...
   * ******************************************************************************************** */
        ...
        override def visitCreateTableClauses(ctx: CreateTableClausesContext): TableClauses = {
                ...
                val serdeInfo =
                        getSerdeInfo(ctx.rowFormat.asScala, ctx.createFileFormat.asScala, ctx)
